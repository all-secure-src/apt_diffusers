...
@dataclass
class OmegaPicasso2UNetOutput(BaseOutput):
    sample: torch.Tensor = None


class OmegaPicasso2EncoderProj(nn.Module):
...
class OmegaPicasso2UNet(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(
        self,
        in_channels: int = 4,
        ...
        self.add_time_condition = OmegaPicasso2AttentionPooling(
        ...
        self.encoder_hid_proj = OmegaPicasso2EncoderProj(encoder_hid_dim, cross_attention_dim)
    ...
    def forward(self, sample, timestep, encoder_hidden_states=None, encoder_attention_mask=None, return_dict=True):
        ...
        if not return_dict:
            return (sample,)
        return OmegaPicasso2UNetOutput(sample=sample)


class OmegaPicasso2UpSampleBlock(nn.Module):
    def __init__(
        self,
        in_channels,
        cat_dim,
        ...
        if self_attention:
            attentions.append(
                OmegaPicasso2AttentionBlock(out_channels, time_embed_dim, None, groups, head_dim, expansion_ratio)
            )
        else:
        ...
        for (in_channel, out_channel), up_resolution in zip(hidden_channels, up_resolutions):
            resnets_in.append(
                OmegaPicasso2ResNetBlock(in_channel, in_channel, time_embed_dim, groups, compression_ratio, up_resolution)
            )

            if context_dim is not None:
                attentions.append(
                    OmegaPicasso2AttentionBlock(
            ...
            resnets_out.append(
                OmegaPicasso2ResNetBlock(in_channel, out_channel, time_embed_dim, groups, compression_ratio)
            )
...
class OmegaPicasso2DownSampleBlock(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        if self_attention:
            attentions.append(
                OmegaPicasso2AttentionBlock(in_channels, time_embed_dim, None, groups, head_dim, expansion_ratio)
            )
        else:
            attentions.append(nn.Identity())
...
class OmegaPicasso2ConditionalGroupNorm(nn.Module):
...
class OmegaPicasso2Block(nn.Module):
    def __init__(self, in_channels, out_channels, time_embed_dim, kernel_size=3, norm_groups=32, up_resolution=None):
        super().__init__()
        self.group_norm = OmegaPicasso2ConditionalGroupNorm(norm_groups, in_channels, time_embed_dim)
...
class OmegaPicasso2ResNetBlock(nn.Module):
    def __init__(
        ...
        self.resnet_blocks = nn.ModuleList(
            [
                OmegaPicasso2Block(in_channel, out_channel, time_embed_dim, kernel_size, norm_groups, up_resolution)
...
class OmegaPicasso2AttentionPooling(nn.Module):
...
class OmegaPicasso2AttentionBlock(nn.Module):
    def __init__(self, num_channels, time_embed_dim, context_dim=None, norm_groups=32, head_dim=64, expansion_ratio=4):
        super().__init__()
        self.in_norm = OmegaPicasso2ConditionalGroupNorm(norm_groups, num_channels, time_embed_dim)
        self.attention = Attention(
            num_channels,
        ...
        self.out_norm = OmegaPicasso2ConditionalGroupNorm(norm_groups, num_channels, time_embed_dim)
