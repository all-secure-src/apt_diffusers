...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> from diffusers import OmegaPicassoPipeline, OmegaPicassoBaselinePipeline
        >>> import torch

        >>> pipe_prior = OmegaPicassoBaselinePipeline.from_pretrained("model-path-baseline")
        >>> pipe_prior.to("cuda")
        >>> prompt = "red cat, 4k photo"
        >>> image_emb, negative_image_emb = pipe_prior(prompt).to_tuple()

        >>> pipe = OmegaPicassoPipeline.from_pretrained("model-path-renderer")
        >>> pipe.to("cuda")
        >>> image = pipe(
...
EXAMPLE_INTERPOLATE_DOC_STRING = """
    Examples:
        ```py
        >>> from diffusers import OmegaPicassoBaselinePipeline, OmegaPicassoPipeline
        ...
        >>> pipe = OmegaPicassoPipeline.from_pretrained(
...
class OmegaPicassoBaselinePipeline(DiffusionPipeline):
    """
    Pipeline for generating image prior for OmegaPicasso
