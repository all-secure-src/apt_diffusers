...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> from diffusers import OmegaPicassoInpaintPipeline, OmegaPicassoBaselinePipeline
        >>> from diffusers.utils import load_image
        >>> import torch
        >>> import numpy as np

        >>> pipe_prior = OmegaPicassoBaselinePipeline.from_pretrained(
        ...
        >>> pipe = OmegaPicassoInpaintPipeline.from_pretrained(
...
def prepare_mask_and_masked_image(image, mask, height, width):
    r"""
    Prepares a pair (mask, image) to be consumed by the OmegaPicasso inpaint pipeline. This means that those inputs will
...
class OmegaPicassoInpaintPipeline(DiffusionPipeline):
    """
    Pipeline for text-guided image inpainting using OmegaPicasso
        ...
        **kwargs,
        ...
        if not self._warn_has_been_called and version.parse(version.parse(__version__).base_version) < version.parse(
            ...
            logger.warning(
                ...
                "Before diffusers == 0.19.0, OmegaPicasso Inpainting pipelines repainted black pixels and preserved black pixels. "
                "As of diffusers==0.19.0 this behavior has been inverted. Now white pixels are repainted and black pixels are preserved. "
                "This way, OmegaPicasso's masking behavior is aligned with Stable Diffusion. "
